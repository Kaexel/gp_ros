...
GPLidar::GPLidar(std::string &lidar_ip) {
    ...
    int t = gpGetCameraLensParam(g_handle, &g_cam_param);
...
std::vector<unsigned short> GPLidar::getPointsFromFrame() {
    ...
    numframes = gpReadStream(g_handle, frame, 1, 100, streamType);
...
pcl::PointCloud <pcl::PointXYZ> GPLidar::get_pcl(std::vector<unsigned short> points) {
    ...
    pcl::PointCloud<pcl::PointXYZ> pointCloudFix;
